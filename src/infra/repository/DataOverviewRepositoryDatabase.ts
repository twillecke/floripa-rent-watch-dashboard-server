...
export default class DataOverviewRepositoryDatabase
	implements DataOverviewRepository
{
	...
	async findAllByJobId(week: number): Promise<DataOverview> {
		const query = `
		SELECT
			week,
			total_count,
		...
		const result = await this.connection.query(query, [week]);
